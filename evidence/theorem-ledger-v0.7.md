...
| T-11 | Given Boolean circuits `O` and `f`, deciding whether some pair has equal observations and unequal task values is NP-complete; deciding exact identifiability is coNP-complete. | Established elementary classification; not a substantive novelty claim | The paper's SAT reduction is cross-checked by the linear-size reduction `G=0`, `f_C=b AND C` from standard Circuit-SAT. Böhler et al. explicitly state Circuit-SAT is NP-complete; Jordan states Boolean-circuit equivalence is coNP-complete. Receipts: `literature/observation-factorization/revision-6-complexity-assessment.json`. The bounded search did not establish historical absence of the exact BAD-FIBER name. |
